...
for i in range(4):
    for j in range(13):
        if board_status_data["Players"][name_index]["Hand"][0]["Suit"] == suits[i] and \
                board_status_data["Players"][name_index]["Hand"][0]["Rank"] == ranks[j]:
            a1 = i
            b1 = j
        elif board_status_data["Players"][name_index]["Hand"][1]["Suit"] == suits[i] and \
                board_status_data["Players"][name_index]["Hand"][1]["Rank"] == ranks[j]:
            a2 = i
            b2 = j
...
for i in range(index_community_cards):
    for w in range(4):
        for j in range(13):
            if board_status_data["CommunityCards"][i]["Suit"] == suits[w] and \
                    board_status_data["CommunityCards"][i]["Rank"] == ranks[j]:
...
bet_input = ""
bet_input_rect = pygame.Rect(screen_width // 5 - 20, screen_height - 260, 400, 50)
bet_input_active = False

# Create confirm and cancel buttons
button_confirm = pygame.Rect(screen_width // 5 + 200, screen_height - 260, 100, 50)
button_cancel = pygame.Rect(screen_width // 5 + 320, screen_height - 260, 100, 50)

# Create X button
button_x = pygame.Rect(screen_width // 5 + 460, screen_height - 260, 50, 50)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if button_check.collidepoint(event.pos):
                send_board_status()
            elif button_bet.collidepoint(event.pos):
                show_bet_buttons = True
            # Check if the text input field is clicked
            elif text_input_rect.collidepoint(event.pos):
            ...
            if text_input_active:
                if event.key == pygame.K_RETURN:
                    # Logic to send the text message
                    text_input_active = False
    ...
    draw_bet_input_field()

    # Draw confirm, cancel, and X buttons if bet button is clicked
    if show_bet_buttons:
        # Draw the bet input field
        draw_bet_input_field()
        draw_rounded_rect(screen, button_confirm, GREEN, 20)
        draw_rounded_rect(screen, button_cancel, RED, 20)
        draw_rounded_rect(screen, button_x, BLACK, 20)
        text_confirm = button_font.render("Confirm", True, WHITE)
        text_cancel = button_font.render("Cancel", True, WHITE)
        text_x = button_font.render("X", True, WHITE)
        screen.blit(text_confirm, (button_confirm.centerx - text_confirm.get_width() // 2,
                                   button_confirm.centery - text_confirm.get_height() // 2))
        screen.blit(text_cancel, (button_cancel.centerx - text_cancel.get_width() // 2,
                                  button_cancel.centery - text_cancel.get_height() // 2))
        screen.blit(text_x, (button_x.centerx - text_x.get_width() // 2,
                             button_x.centery - text_x.get_height() // 2))
    pygame.display.flip()
